...
const swapIndicesAndSpaces = arr => {
...
const playTicTacToe = (board, next) => {
  ...
  if (
    board.filter(
      square => square !== 'x' && square !== 'o' && !Number.isInteger(square)
    ).length > 0
  ) {
...
const generateScore = (squares, board) => {
...
const miniMax = (board, player = computer) => {
...
const filterBoard = (board, val = 'int') => {
  // default val to integers
